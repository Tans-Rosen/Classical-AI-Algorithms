...
class CrawlingRobotEnvironment():
    ...
    def getAllStates(self):
        states = []
        for arm in range(len(self.armBuckets)):
            for hand in range(len(self.handBuckets)):
                states.append((arm, hand))
        return states
    ...
    def transition(self, state, action):
        """
         ...
         Returns:
           nextState, reward
        """
        armBucket, handBucket = state
        self.crawlingRobot.setAngles(self.armBuckets[armBucket], self.handBuckets[handBucket])
...
class CrawlingRobot:
    ...
    def __init__(self, canvas):
        ...
        self.ground = canvas.create_rectangle(0,
            self.groundY,self.totWidth,self.totHeight, fill='white')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Crawler Robot",
    )
    parser.add_argument(
        "-q", action='store_true', help="Train robot using Q-learning"
    )
