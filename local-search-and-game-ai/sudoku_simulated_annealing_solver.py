...
def simulated_annealing(board, clues, startT, decay, tol=1e-4):
    ...
    while(True):
        T = startT * (decay**iter)
        succs = successors(board, clues)

        if T < tol or len(succs) == 0 or num_errors(board) == 0:
            break

        next_suc = random.choice(succs)
        deltaE = num_errors(board) - num_errors(next_suc)
        if deltaE > 0:
            board = next_suc
        ...
        iter += 1
        errors.append(num_errors(board))
